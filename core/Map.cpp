...
bool Map::isInView(Drawable* d) {
	return   (d->getPosition()->getX() + d->getImage()->getWidth()  > viewX*unit) &&
			 (d->getPosition()->getX() < viewX*unit + sizeViewX*unit) &&
		     (d->getPosition()->getY() + d->getImage()->getHeight() > viewY*unit) &&
		     (d->getPosition()->getY() < viewY*unit + sizeViewY*unit);
}
